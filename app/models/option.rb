  ...
  def self.get( option_name )
    begin
      ...
      if option_name == "permalink_structure"
        ...
        raise StandardError, "Option '#{option_name}' not found. Check option name and try again. May not be a valid Wordpress option."
      end
    end
  end
  
  def self.permalink_route
    option = self.get( "permalink_structure" )
