class Post < ActiveRecord::Base
  ...
  def is_live?
    self.post_status == "publish" ? true : false
  end
  ...
  def permalink
    ...
    structure.each do |part|
      case part
      when "%year%"
        plink << self.post_date.to_s[0,4]
      when "%monthnum%"
        plink << self.post_date.to_s[5,2]
      when "%day%"
        plink << self.post_date.to_s[8,2]
      when "%hour%"
      when "%minute%"
      when "%second%"
      when "%postname%"
        plink << self.post_name
      when "%post_id%"
      when "%category%"
      when "%tag%"
      when "%author%"
  ...
  def self.get_from_permalink( params )    
    ...
    #TODO TBH, I think the method below is faster, because it's a straight SQL call. But it relies on the date being present in the permalink.
    if post.nil?
      post = Post.published.where( :post_name => params[ "postname" ]).where( "post_date LIKE ?",  "%#{params['year']}-#{params['monthnum']}-#{params['day']}%" ).first
    end
    return post
  end
